...
@click.option("--data-file-name", type=str, default='20190621_BTSP_CA1_data_calibrated_input.hdf5')
...
def main(cli, min_delta_weight, peak_delta_weight, input_field_width, calibration_ramp_width, data_dir, data_file_name,
         export_file_name, export, plot, verbose):
    """
...
def init_context():
    ...
    if context.export_file_name is None:
        context.export_file_path = None
    else:
        context.export_file_path = context.data_dir + '/' + context.export_file_name
    ...
    with h5py.File(context.data_file_path, 'r') as f:
        ...
        ramp_scaling_factor = \
            calibrate_ramp_scaling_factor(ramp_x=binned_x, input_x=binned_x, interp_x=default_interp_x,
                                          input_rate_maps=input_rate_maps, peak_locs=peak_locs,
                                          track_length=track_length, target_range=context.target_range,
                                          bounds=(0., context.peak_delta_weight),
                                          calibration_ramp_width=context.calibration_ramp_width,
                                          plot=context.plot > 0, verbose=context.verbose)
        if context.verbose > 1:
            print('get_LSA_weights_BTSP_CA1: ramp_scaling_factor recomputed for %.1f cm field width: %.4E' %
...
def import_data(cell_id, induction):
    ...
    with h5py.File(context.data_file_path, 'r') as f:
        if cell_key not in f['data'] or induction_key not in f['data'][cell_key]:
            raise KeyError('get_LSA_weights_BTSP_CA1: no data found for cell_id: %s, induction: %s' %
    ...
    if context.verbose > 0:
        print('get_LSA_weights_BTSP_CA1: process: %i loaded data for cell: %i, induction: %i' %
...
def export_data(export_file_path=None):
    ...
    with h5py.File(export_file_path, 'a') as f:
        ...
        if induction_key in f['calibrated_input'][input_field_width_key][cell_key]:
            raise RuntimeError('get_LSA_weights_BTSP_CA1: data for input_field_width: %.1f; cell: %s; induction: %s '
