...
Features/assumptions of voltage-dependent model G:
...
biBTSP_VD_G:
No gain is applied to plasticity signal overlap.
"""
...
def plot_model_summary_supp_figure(cell_id, export_file_path=None, exported_data_key=None, induction_lap=0):
    ...
    clean_axes(axes)
    fig.suptitle('Voltage-dependent model G (cell %i)' % cell_id,
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_VD_G_cli_config.yaml')
...
def main(cli, config_file_path, output_dir, export, export_file_path, label, verbose, plot, interactive, debug,
    ...
    python -i optimize_biBTSP_VD_G.py --cell_id=1 --plot --framework=serial --interactive
    ...
    python -i optimize_biBTSP_VD_G.py --param_file_path=$PATH_TO_PARAM_YAML --model_key=$VALID_KEY_IN_PARAM_YAML \
