...
Features/assumptions of weight-dependent model E:
...
biBTSP_WD_E: Single eligibility signal filter. Sigmoidal f_pot and f_dep.
"""
...
def calculate_model_ramp(local_signal_peak=None, global_signal_peak=None, model_id=None, export=False, plot=False):
    ...
    if plot and context.induction == 1:
        ...
        axes[1].set_xlabel('Eligibility signal amplitude (a.u.)')
...
def plot_model_summary_supp_figure(cell_id, export_file_path=None, exported_data_key=None, induction_lap=0):
    ...
    clean_axes(axes)
    fig.suptitle('Weight-dependent model E (cell %i)' % cell_id,
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_WD_E_cli_config.yaml')
...
def main(cli, config_file_path, output_dir, export, export_file_path, label, verbose, plot, interactive, debug,
    ...
    python -i optimize_biBTSP_WD_E.py --cell_id=1 --plot --framework=serial --interactive
    ...
    python -i optimize_biBTSP_WD_E.py --param_file_path=$PATH_TO_PARAM_YAML --model_key=$VALID_KEY_IN_PARAM_YAML \
