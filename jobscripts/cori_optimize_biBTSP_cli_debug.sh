...
export JOB_NAME=optimize_biBTSP_"$3"_cell"$1"_"$DATE"_debug
export cores=$(($2 * 32))
sbatch <<EOT
#!/bin/bash -l
#SBATCH -J $JOB_NAME
...
srun -N $2 -n $cores -c 2 --cpu-bind=cores python -m mpi4py.futures -m nested.optimize \
    ...
    --pop_size=200 --max_iter=2 --path_length=1 --disp --export --label="$4"cm_cell"$1"_debug --cell_id=$1 \
    --framework=mpi --input_field_width=$4
EOT
