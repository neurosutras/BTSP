...
gating signal, can activate a forward process to increase the number of AMPA-Rs stably incorporated into synapses.
4) Activity at each synapse generates a local de-potentiation eligibility signal that, in conjunction with the global
gating signal, can activate a reverse process to decrease the number of AMPA-Rs stably incorporated into synapses.
5) AMPAR-s can be in 2 states (Markov-style kinetic scheme):

     rMC0 * global_signal * f_pot(pot_signal)
M (mobile) <----------------------> C (captured by a synapse)
     rCM0 * global_signal * f_depot(depot_signal)
...
9) f_pot represents the "sensitivity" of the forward process to the presence of the pot_signal. The transformation
f_pot is linear.
10) f_depot represents the "sensitivity" of the reverse process to the presence of the depot_signal. The transformation
f_depot is linear.

biBTSP_G vs. biBTSP_D: Potentiation and de-potentiation are driven by separate eligibility signals. f_pot and f_depot
are linear.
"""
...
def calculate_model_ramp(pot_signal_peak=None, depot_signal_peak=None, global_signal_peak=None, export=False,
    ...
    pot_signals = \
        np.divide(get_local_signal_population(pot_signal_filter, context.down_rate_maps, context.down_dt),
                  pot_signal_peak)
    depot_signals = \
        np.divide(get_local_signal_population(depot_signal_filter, context.down_rate_maps, context.down_dt),
                  depot_signal_peak)

    signal_xrange = np.linspace(0., 1., 10000)
    pot_rate = lambda x: x
    depot_rate = lambda x: x
    if plot:
        fig, axes = plt.subplots(1)
    ...
    for induction_lap in range(len(context.induction_start_times)):
        ...
        next_weights = []
        for i, (this_pot_signal, this_depot_signal) in enumerate(zip(pot_signals, depot_signals)):
            this_pot_rate = np.trapz(np.multiply(pot_rate(this_pot_signal[indexes]), global_signal[indexes]),
                                     dx=context.down_dt / 1000.)
            this_depot_rate = np.trapz(np.multiply(depot_rate(this_depot_signal[indexes]), global_signal[indexes]),
                                       dx=context.down_dt / 1000.)
...
def plot_model_summary_figure(cell_id, model_file_path=None):
    ...
    axes[0][1].set_title('Linear q$_{+}$, linear q$_{-}$', fontsize=mpl.rcParams['font.size'], pad=10.)
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_G_90cm_cli_config.yaml')
...
def main(cli, config_file_path, output_dir, export, export_file_path, label, verbose, plot, interactive, debug,
    ...
    python -i optimize_biBTSP_G.py --cell_id=1 --plot --framework=serial --interactive

    To execute using MPI parallelism with 1 controller process and N - 1 worker processes:
    mpirun -n N python -i -m mpi4py.futures optimize_biBTSP_G.py --cell_id=1 --plot --framework=mpi -interactive
    ...
    python -i optimize_biBTSP_G.py --cell_id=1 --plot-summary-figure --model-file-path=$PATH_TO_MODEL_FILE \
