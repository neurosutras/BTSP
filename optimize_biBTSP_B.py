...
f_depot is the product of one ascending and one descending sigmoid.

biBTSP_B vs. biBTSP_F: f_depot is a double sigmoid.
"""
...
def calculate_model_ramp(pot_signal_peak=None, depot_signal_peak=None, global_signal_peak=None, export=False,
    ...
    try:
        ...
        if context.verbose > 0:
            print('optimize_biBTSP_%s: calculate_model_ramp: pid: %i ; aborting - invalid parameters for depot_rate' %
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_B_90cm_cli_config.yaml')
...
def main(cli, config_file_path, output_dir, export, export_file_path, label, verbose, plot, interactive, debug,
    ...
    python -i optimize_biBTSP_B.py --cell_id=1 --plot --framework=serial --interactive

    To execute using MPI parallelism with 1 controller process and N - 1 worker processes:
    mpirun -n N python -i -m mpi4py.futures optimize_biBTSP_B.py --cell_id=1 --plot --framework=mpi -interactive
    ...
    python -i optimize_biBTSP_B.py --cell_id=1 --plot-summary-figure --model-file-path=$PATH_TO_MODEL_FILE \
