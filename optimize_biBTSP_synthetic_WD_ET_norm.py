...
def load_data(induction):
    ...
    if induction in context.data_cache:
        ...
        if induction == 2:
            ...
            target_delta_ramp_dep = get_target_synthetic_ramp(induction_loc_1, ramp_x=context.binned_x,
                                                              track_length=context.track_length,
                                                              target_peak_val=-context.target_dep_delta_peak_val_2,
                                                              target_min_val=0.,
                                                              target_asymmetry=context.target_asymmetry_2,
                                                              target_peak_shift=context.target_peak_shift,
                                                              target_ramp_width=context.target_ramp_dep_width_2)
            target_delta_ramp_pot = get_target_synthetic_ramp(induction_context.mean_induction_start_loc,
                                                              ramp_x=context.binned_x,
                                                              track_length=context.track_length,
                                                              target_peak_val=context.target_pot_delta_peak_val_2,
                                                              target_min_val=0.,
                                                              target_asymmetry=context.target_asymmetry_2,
                                                              target_peak_shift=context.target_peak_shift,
                                                              target_ramp_width=context.target_ramp_pot_width_2)
            target_delta_ramp = np.subtract(target_delta_ramp_pot, target_delta_ramp_dep)
            target_ramp_2 = np.add(induction_context.target_ramp['before'], target_delta_ramp)
        ...
        if context.plot:
            ...
            axes[0].plot(context.binned_x, induction_context.target_ramp['before'], c='grey', label='Before')
            axes[0].plot(context.binned_x, induction_context.target_ramp['after'], c='k', label='After')
            axes[0].set_xlabel('Position (cm)')
            axes[0].set_ylabel('Ramp amplitude (mV)')
            axes[1].plot(induction_context.min_induction_t[induction_context.clean_induction_t_indexes] / 1000.,
                         np.subtract(induction_context.target_ramp['after'],
                                     induction_context.target_ramp['before'])[
                             induction_context.clean_induction_t_indexes], c='k')
            axes[1].set_xlabel('Time (s)')
            axes[1].set_ylabel('Change in ramp\namplitude (mV)')
            clean_axes(axes)
            fig.suptitle('Induction %i' % induction)
            fig.tight_layout()
            fig.show()
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_synthetic_WD_ET_norm_config.yaml')
