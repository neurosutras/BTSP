...
#@click.option("--vmin", type=float, default=-6.33195)
#@click.option("--vmax", type=float, default=7.14836)
...
def main(data_file_path, model_file_path, output_dir, vmin, vmax, export, debug, label, target_induction):
    """
    ...
    if export and not os.path.isdir(output_dir):
        raise IOError('plot_biBTSP_data_summary_figure: invalid output_dir: %s' % output_dir)
    ...
    pop_mean_min_delta_t = defaultdict(list)
    pop_mean_delta_exp_ramp = defaultdict(list)

    # fig, axes = plt.figure(figsize=(7, 6)), []
    fig, axesgrid = plt.subplots(2, 2, figsize=(8.25, 6))
    # gs0 = gridspec.GridSpec(2, 2, wspace=0.7, hspace=0.5, left=0.2, right=0.9, top=0.90, bottom=0.15)
    axes = []
    for row in range(2):
        for col in range(2):
            # axes.append(fig.add_subplot(gs0[col, row]))
    ...
    if vmin is None or vmax is None:
        vmax = np.max(flat_delta_ramp)
        vmin = np.min(flat_delta_ramp)
    cmax = max(abs(vmax), abs(vmin))
    ...
    for cell_key in (cell_key for cell_key in interp_delta_exp_ramp
        ...
        lc = colorline(reference_delta_t, interp_delta_exp_ramp[cell_key][induction_key],
                       interp_initial_exp_ramp[cell_key][induction_key], vmin=0., vmax=ymax, cmap=lines_cmap)
    ...
    cax = axes[3].pcolor(t_grid, initial_ramp_grid, interp_data, cmap=interp_cmap, vmax=cmax, vmin=-cmax, zorder=0)
    ...
    print('3D data color legend: vmin: %.5f, vmax: %.5f' % (vmin, vmax))
    clean_axes(axes)
