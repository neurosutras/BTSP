...
Features/assumptions of voltage-dependent model C:
...
biBTSP_VD_C: Dual eligibility signal filters. Sigmoidal f_pot and f_dep.
L_pot ~ pot_signal_filter(pre_rate * (1. - V / V_max))
L_dep ~ dep_signal_filter(pre_rate *  V / V_max)
...
def calculate_model_ramp(pot_signal_peak=None, dep_signal_peak=None, global_signal_peak=None, export=False,
    ...
    for induction_lap in range(len(context.induction_start_times)):
        ...
        dep_eligibility_signals = np.divide(
            get_voltage_dependent_eligibility_signal_population(dep_signal_filter, current_complete_normalized_ramp,
    ...
    if export:
        with h5py.File(context.temp_output_path, 'a') as f:
            ...
            group.attrs['dep_signal_peak'] = dep_signal_peak
...
def plot_model_summary_figure(cell_id, model_file_path=None):
    ...
    clean_axes(axes)
    fig.suptitle('Voltage-dependent model C; cell: %i, induction: %i' % (cell_id, 2),
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_VD_C_cli_config.yaml')
...
def main(cli, config_file_path, output_dir, export, export_file_path, label, verbose, plot, interactive, debug,
    ...
    python -i optimize_biBTSP_VD_C.py --cell_id=1 --plot --framework=serial --interactive

    To execute using MPI parallelism with 1 controller process and N - 1 worker processes:
    mpirun -n N python -i -m mpi4py.futures optimize_biBTSP_VD_C.py --cell_id=1 --plot --framework=mpi --interactive
    ...
    python -i optimize_biBTSP_VD_C.py --cell_id=1 --plot-summary-figure --model-file-path=$PATH_TO_MODEL_FILE \
