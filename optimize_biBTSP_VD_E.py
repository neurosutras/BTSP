...
biBTSP_VD_E: Single eligibility signal filter. Sigmoidal f_pot and f_dep.
L_pot ~ local_signal_filter(pre_rate * (1. - V / V_max))
L_dep ~ local_signal_filter(pre_rate *  V / V_max)
dW/dt ~ k_pot * global_signal * f_pot(L_pot) - k_dep * global_signal * f_dep(L_dep)
...
def calculate_model_ramp(local_signal_peak=None, global_signal_peak=None, model_id=None, export=False, plot=False):
    ...
    if plot:
        ...
        axes.set_ylabel('Instructive signal amplitude (a.u.)')
    ...
    for induction_lap in range(len(context.induction_start_times)):
        ...
        if plot:
            axes.plot(context.down_t[indexes] / 1000., global_signal[indexes])
...
def plot_model_summary_supp_figure(cell_id, export_file_path=None, exported_data_key=None, induction_lap=0):
    ...
    for i, (name, index) in enumerate(viewitems(example_input_dict)):
        ...
        axes[1][i + 1].plot(this_t, this_pot_signal, c=colors[0], label='Synaptic potentiation eligibility signal',
                            linewidth=1.)
        axes[1][i + 1].plot(this_t, this_dep_signal, c=colors[1], label='Synaptic depression eligibility signal',
                            linewidth=1.)
    ...
    ymax0_right = max(10., this_peak_ramp_amp / 0.85)
    ymax1 /= 0.85
...
def plot_model_summary_figure(cell_id, export_file_path=None, exported_data_key=None, induction_lap=0,
    ...
    axes[1].plot(this_t, example_pot_elig_signal, c='r', linewidth=1., label='Potentiation eligibility')
    axes[1].plot(this_t, example_dep_elig_signal, c='c', linewidth=1.,
                 label='Depression eligibility')
    ...
    axes[1].fill_between(this_t, 0., np.minimum(example_pot_elig_signal, this_global_signal), alpha=0.5,
                         facecolor='r', edgecolor='none', label='Potentiation signal overlap', linewidth=0.)
    axes[1].fill_between(this_t, 0., np.minimum(example_dep_elig_signal, this_global_signal),
                         alpha=0.5, facecolor='c', edgecolor='none', label='Depression signal overlap', linewidth=0.)
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_VD_E_cli_config.yaml')
...
def main(cli, config_file_path, output_dir, export, export_file_path, label, verbose, plot, interactive, debug,
    ...
    python -i optimize_biBTSP_VD_E.py --cell_id=1 --plot --framework=serial --interactive
    ...
    python -i optimize_biBTSP_VD_E.py --param_file_path=$PATH_TO_PARAM_YAML --model_key=$VALID_KEY_IN_PARAM_YAML \
