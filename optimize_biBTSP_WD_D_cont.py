...
biBTSP_WD_D_cont: Single eligibility signal filter. Sigmoidal f_pot and f_dep.
Weights are updated instantaneously, rather than once per lap.
"""
...
def calculate_model_ramp(local_signal_peak=None, global_signal_peak=None, model_id=None, export=False, plot=False):
    ...
    signal_overlap = local_signals * global_signal
    for induction_lap in range(len(context.induction_start_times)):
        ...
        next_normalized_weights = np.copy(current_normalized_weights)
        for i in indexes:
            norm_dw_dt = context.k_pot * (1. - next_normalized_weights) * pot_rate(signal_overlap[:,i]) - \
                         context.k_dep * next_normalized_weights * dep_rate(signal_overlap[:,i])
            next_normalized_weights += norm_dw_dt * context.down_dt / 1000.
            next_normalized_weights = np.maximum(0., np.minimum(1., next_normalized_weights))
        if plot:
...
def plot_model_summary_figure(cell_id, export_file_path=None, exported_data_key=None, induction_lap=0,
    ...
    for lap, this_induction_lap in enumerate(range(len(context.induction_start_times))):
        ...
        for i in indexes:
            norm_dw_dt = context.k_pot * (1. - next_normalized_weights) * pot_rate(signal_overlap[:, i]) - \
                         context.k_dep * next_normalized_weights * dep_rate(signal_overlap[:, i])
            complete_change_in_weight_matrix[:, i] = norm_dw_dt * peak_weight
            next_normalized_weights += norm_dw_dt * context.down_dt / 1000.
            next_normalized_weights = np.maximum(0., np.minimum(1., next_normalized_weights))
...
def filter_features_model_ramp(primitives, current_features, model_id=None, export=False, plot=False):
    """
...
def get_objectives(features, model_id=None, export=False, plot=False):
    ...
    if context.objective_names is not None:
...
def run_tests():
    ...
    new_features = context.interface.execute(filter_features_model_ramp, primitives, features, model_id,
                                             context.export)
    features.update(new_features)
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_WD_D_cont_cli_config.yaml')
