...
def get_residual_score(delta_weights, target_ramp, binned_x, interp_x, induction_loc, track_length, target_range,
    ...
    peak_loc['target'], end_loc['target'], min_val['target'], min_loc['target'] = \
        calculate_ramp_features(ramp=exp_ramp, induction_loc=induction_loc, binned_x=binned_x, interp_x=interp_x,
    ...
    peak_loc['model'], end_loc['model'], min_val['model'], min_loc['model'] = \
        calculate_ramp_features(ramp=model_ramp, induction_loc=induction_loc, binned_x=binned_x, interp_x=interp_x,
                                track_length=track_length)
...
def get_weights_LSA_scaling_factor(local_context, input_rate_maps, bounds=None, beta=2., ramp_x=None, input_x=None,
    ...
    """
    if ramp_x is None:
        ramp_x = local_context.binned_x
    if input_x is None:
        input_x = local_context.binned_x
    modulated_field_center = local_context.track_length * 0.5
    local_context.mean_induction_start_loc = modulated_field_center + 10.
    local_context.mean_induction_stop_loc = local_context.mean_induction_start_loc + 5.
    peak_delta_weight = 1.5
    tuning_amp = peak_delta_weight / 2.
    tuning_offset = tuning_amp
    force_delta_weights = tuning_amp * np.cos(2. * np.pi / (local_context.input_field_width * 1.2) *
                                              (local_context.peak_locs - modulated_field_center)) + tuning_offset
    left = np.where(local_context.peak_locs >= modulated_field_center - local_context.input_field_width * 1.2 / 2.)[0][0]
    right = np.where(local_context.peak_locs > modulated_field_center + local_context.input_field_width * 1.2 / 2.)[0][0]
    force_delta_weights[:left] = 0.
    force_delta_weights[right:] = 0.
    exp_ramp = force_delta_weights.dot(input_rate_maps)
    ramp_scaling_factor = 6. / np.max(exp_ramp)
    input_matrix = np.multiply(input_rate_maps, local_context.ramp_scaling_factor)
    ...
    ramp_amp['target'], ramp_width['target'], peak_shift['target'], ratio['target'], start_loc['target'], \
    peak_loc['target'], end_loc['target'], min_val['target'], min_loc['target'] = \
        calculate_ramp_features(ramp=target_ramp, induction_loc=local_context.mean_induction_start_loc,
                                binned_x=local_context.binned_x, interp_x=local_context.default_interp_x,
    ...
    if bounds is None:
        bounds = [local_context.min_delta_weight, 3.]
    result = minimize(get_residual_score, delta_weights,
    ...
    ramp_amp['model'], ramp_width['model'], peak_shift['model'], ratio['model'], start_loc['model'], \
    peak_loc['model'], end_loc['model'], min_val['model'], min_loc['model'] = \
        calculate_ramp_features(ramp=model_ramp, induction_loc=local_context.mean_induction_start_loc,
                                binned_x=local_context.binned_x, interp_x=local_context.default_interp_x,
                                track_length=local_context.track_length)
    ...
    if plot:
        x_start = local_context.mean_induction_start_loc
        x_end = local_context.mean_induction_stop_loc
        ...
        axes.set_xlim([0., local_context.track_length])
        ...
        axes1.plot(local_context.peak_locs, force_delta_weights + 1., label='Target', c='k')
        axes1.plot(local_context.peak_locs, SVD_delta_weights + 1., c='r', label='Model (SVD)')
        axes1.plot(local_context.peak_locs, delta_weights + 1., c='c', label='Model (LSA)')
        ...
        axes1.set_xlim([0., local_context.track_length])
