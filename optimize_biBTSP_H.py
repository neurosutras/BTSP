...
2) Activity at each synapse generates long duration 'local plasticity signals', or 'eligibility traces' for
synaptic plasticity.
...
is inversely proportional to the local voltage in the postsynaptic spine.
3) Activity at each synapse generates a local depression signal that, in conjunction with the global gating signal,
can activate a reverse process to decrease synaptic strength. The amplitude of this eligibility signal is proportional
to the local voltage in the postsynaptic spine.
...
6) f_pot represents the "sensitivity" of the forward process to the presence of the local potentiation eligibility
signal. The transformation f_pot has the flexibility to be any segment of a sigmoid (so can be linear,
exponential rise, or saturating).
7) f_dep represents the "sensitivity" of the reverse process to the presence of the local depression signal. The
transformation f_dep has the flexibility to be any segment of a sigmoid (so can be linear, exponential rise, or
saturating).

biBTSP_H vs. biBTSP_D: Weights affect spine voltage and eligibility signal amplitude, rather than the conversion of
eligibility signals to weight changes.
...
def calculate_model_ramp(local_signal_peak=None, global_signal_peak=None, export=False, plot=False):
    ...
    for induction_lap in range(len(context.induction_start_times)):
        ...
        for i, this_local_signal in enumerate(local_signals):
            this_pot_rate = \
                np.trapz(
                    np.multiply(pot_rate(this_local_signal[indexes] * (1. - current_normalized_weights[i])),
                                global_signal[indexes]),
                    dx=context.down_dt / 1000.)
            this_dep_rate = \
                np.trapz(
                    np.multiply(dep_rate(this_local_signal[indexes] * current_normalized_weights[i]),
                                global_signal[indexes]),
                    dx=context.down_dt / 1000.)
            this_normalized_delta_weight = context.k_pot * this_pot_rate - context.k_dep * this_dep_rate
...
@click.option("--config-file-path", type=click.Path(exists=True, file_okay=True, dir_okay=False),
              default='config/optimize_biBTSP_H_90cm_cli_config.yaml')
...
def main(cli, config_file_path, output_dir, export, export_file_path, label, verbose, plot, interactive, debug,
    ...
    python -i optimize_biBTSP_H.py --cell_id=1 --plot --framework=serial --interactive

    To execute using MPI parallelism with 1 controller process and N - 1 worker processes:
    mpirun -n N python -i -m mpi4py.futures optimize_biBTSP_H.py --cell_id=1 --plot --framework=mpi -interactive
    ...
    python -i optimize_biBTSP_H.py --cell_id=1 --plot-summary-figure --model-file-path=$PATH_TO_MODEL_FILE \
